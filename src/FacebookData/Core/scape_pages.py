...
from FacebookData.Core.token import Token as tk
from FacebookData.Core.page_datas import PostData
...
class PageData:
    ...
    def scrape(self):
        with open('Data/ListPages_{}.csv'.format(self.__keyWord), 'w') as file:
            w = csv.writer(file)
            ...
            parameters = "&limit={}&access_token={}".format(5, self.__accessToken)

            print("Start: {}".format(scrape_starttime))

            while has_next_page or number_processed <= 1000:
            ...
            print("\nDone!\n{} Page Processed in {}".format(
                number_processed, datetime.datetime.now() - scrape_sratttime))
